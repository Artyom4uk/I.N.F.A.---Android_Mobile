...
public class MainActivity extends Activity {
    ...
    private void diary(){final EditText e=field("Что произошло сегодня?");new AlertDialog.Builder(this).setTitle("📔 Дневник").setView(e).setPositiveButton("Сохранить",(d,w)->{String date=new SimpleDateFormat("dd.MM.yyyy",Locale.getDefault()).format(new Date());prefs.edit().putString("diary",date+"\n"+e.getText()).apply();toast("Запись сохранена");done();}).setNegativeButton("Отмена",(d,w)->done()).show();}
    private void reminder(){final EditText e=field("Текст напоминания");new AlertDialog.Builder(this).setTitle("⏰ Напоминание").setView(e).setPositiveButton("Через 1 час",(d,w)->schedule(e.getText().toString())).setNegativeButton("Отмена",(d,w)->done()).show();}
    private void schedule(String s){if(Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},77);Intent i=new Intent(this,ReminderReceiver.class);i.putExtra("text",s);PendingIntent pi=PendingIntent.getBroadcast(this,(int)System.currentTimeMillis(),i,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);((AlarmManager)getSystemService(ALARM_SERVICE)).set(AlarmManager.RTC_WAKEUP,System.currentTimeMillis()+3600000,pi);toast("Напоминание поставлено");done();}
    private void calc(){final EditText e=field("Например: 12 * 8 + 4");e.setSingleLine();new AlertDialog.Builder(this).setTitle("⌗ Калькулятор").setView(e).setPositiveButton("Посчитать",(d,w)->{try{toast("= "+fmt(new Parser(e.getText().toString()).parse()));}catch(Exception ex){toast("Не удалось посчитать");}done();}).setNegativeButton("Отмена",(d,w)->done()).show();}
    private String fmt(double x){if(x==Math.rint(x))return Long.toString((long)x);return String.format(Locale.US,"%.6f",x).replaceAll("0+$","").replaceAll("\\.$","");}
    private static class Parser{String s;int p;Parser(String s){this.s=s.replace(" ","");}double parse(){double v=e();if(p<s.length())throw new RuntimeException();return v;}double e(){double v=t();while(p<s.length()){char c=s.charAt(p);if(c!='+'&&c!='-')break;p++;double n=t();v=c=='+'?v+n:v-n;}return v;}double t(){double v=f();while(p<s.length()){char c=s.charAt(p);if(c!='*'&&c!='/')break;p++;double n=f();v=c=='*'?v*n:v/n;}return v;}double f(){if(p<s.length()&&s.charAt(p)=='-'){p++;return-f();}int st=p;while(p<s.length()&&(Character.isDigit(s.charAt(p))||s.charAt(p)=='.'))p++;if(st==p)throw new RuntimeException();return Double.parseDouble(s.substring(st,p));}}
    private void phone(){android.os.StatFs st=new android.os.StatFs(getFilesDir().getAbsolutePath());long free=st.getAvailableBytes()/1073741824L,total=st.getTotalBytes()/1073741824L;dialog("📱 Система","Модель · "+Build.MANUFACTURER+" "+Build.MODEL+"\nAndroid · "+Build.VERSION.RELEASE+" / API "+Build.VERSION.SDK_INT+"\nХранилище · "+free+" ГБ свободно из "+total+" ГБ");}
